...
function SelectFilesCard({ isServerStarted, loading, onSelectFiles, onStartEmptyServer }: Pick<LocalShareDashboardProps, 'isServerStarted' | 'loading' | 'onSelectFiles' | 'onStartEmptyServer'>) {
  return (
    <>
    <Pressable
    ...
    {!isServerStarted && (
      <Pressable style={[styles.receiveOnlyButton, loading && styles.dimmed]} onPress={onStartEmptyServer} disabled={loading}>
        <Text style={styles.receiveOnlyButtonText}>Start receiving without files</Text>
        <Text style={styles.receiveOnlyArrow}>→</Text>
      </Pressable>
    )}
    </>
  );
...
const styles = StyleSheet.create({
  ...
  receiveOnlyButton: { minHeight: 48, marginTop: 10, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderRadius: 14, borderWidth: 1, borderColor: '#bfdbfe', backgroundColor: '#eff6ff' }, receiveOnlyButtonText: { color: '#1d4ed8', fontSize: 13, fontWeight: '800' }, receiveOnlyArrow: { color: '#2563eb', fontSize: 20 },
  sectionCard: { marginTop: 18, padding: 18, borderWidth: 1, borderColor: '#e5eaf2', borderRadius: 20, backgroundColor: '#fff' },
